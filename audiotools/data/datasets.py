...
class AudioLoader:
    ...
    def get_multiscale_audio(self, audio, sr=22050, num_scales=5):
        res = [audio]
    ...
    def __call__(
        ...
        if transform is not None:
            transform_args = transform.instantiate(state, signal=signal)  # transform
            signal = transform(signal, **transform_args)

        audio = signal.audio_data
        m_audio = self.get_multiscale_audio(audio, sr=sample_rate, num_scales=3)
        ...
        if transform is not None:
            item["transform_args"] = transform.instantiate(state, signal=signal)
        return item
