...
def load(
    ...
    generator = DAC(use_spec_input=True) if generator is None else generator
...
def train_loop(state, batch, accel, lambdas):
    ...
    for i in range(len(gt_audios)):
        ...
        with accel.autocast():
            ...
            #output.setdefault("stft/loss", []).append(state.stft_loss(recons, gt_signal))
            #output.setdefault("mel/loss", []).append(state.mel_loss(recons, gt_signal, mel_fmax=mel_fmax))
